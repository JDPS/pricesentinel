...
def test_generate_health_summary_warns_on_uncertainty_metrics(tmp_path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    manager = CountryDataManager("XX", base_path="data")
    manager.create_directories()
    ...
    score = pd.DataFrame(
        {
            ...
            "directional_accuracy": [0.7] * 7,
            "peak_hour_abs_error": [8.0] * 7,
            "quantile_coverage_10_90": [0.30] * 7,
            "pinball_loss_avg": [12.0] * 7,
            "interval_width_avg": [10.0] * 7,
            "generated_at_utc": [datetime.now(UTC).isoformat()] * 7,
        }
    )
    score.to_csv(score_dir / "daily_scorecard.csv", index=False)
    ...
    summary = out["summary"]
    assert summary["alerts"]["overall_status"] == "warn"
    codes = {c["code"] for c in summary["alerts"]["conditions"]}
    assert "uncertainty_coverage" in codes
