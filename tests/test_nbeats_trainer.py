...
"""Tests for the N-BEATS deep learning trainer."""
...
def test_train_returns_metrics(
    ...
    x, y = synthetic_data
    trainer = NBEATSTrainer(model_name="nbeats_test", config=tiny_config)
    metrics = trainer.train(x, y)

    assert "mae" in metrics
    assert "model_type" in metrics
    assert "epochs_trained" in metrics
    assert metrics["model_type"] == "nbeats"
...
def test_predict_before_train_raises() -> None:
    ...
    trainer = NBEATSTrainer(model_name="nbeats_test")
...
def test_custom_hyperparameters() -> None:
    ...
    trainer = NBEATSTrainer(model_name="nbeats_test", config=config)
...
def test_save(
    ...
    model_dir = tmp_path / "models" / "XX" / "nbeats_test" / "run_001"  # type: ignore[operator]
