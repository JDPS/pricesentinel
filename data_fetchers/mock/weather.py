...
class MockWeatherFetcher(WeatherDataFetcher):
    ...
    def fetch_weather(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        for coord in self.coordinates:
            ...
            # Random variation
            temp_noise = np.random.normal(0, 2, len(timestamps))
            temperature = base_temp + daily_temp_cycle + seasonal_cycle + temp_noise

            # Wind speed (m/s) - always positive, occasional gusts
            base_wind = 5.0
            wind_variation = np.abs(np.random.normal(0, 3, len(timestamps)))
            # Occasional strong winds
            gust_mask = np.random.random(len(timestamps)) < 0.05
            gusts = np.where(gust_mask, np.random.uniform(10, 20, len(timestamps)), 0)
            ...
            cloud_factor = np.random.uniform(0.5, 1.0, len(timestamps))
            ...
            rain_mask = np.random.random(len(timestamps)) < 0.1  # 10% chance of rain
            precipitation[rain_mask] = np.random.exponential(2, rain_mask.sum())
        ...
        logger.info(
            f"Generated {len(df)} synthetic weather" f" records from {start_date} to {end_date}"
        )
